...
# from vlm_lora.tasks import SequenceClassificationTask, task_dict
# from vlm_lora.utils import is_package_available

# if is_package_available("bitsandbytes"):
#     from transformers import BitsAndBytesConfig
# else:
#     from vlm_lora.utils import BitsAndBytesConfig



# class CasualOutputLayer(VLMOutput):
...
# class ClassificationOutputLayer(VLMOutput):
#     def __init__(
#         self,
#         task_type: str,
#         num_labels: int,
...
#     transformer_layer: VLMDecoder,
#     llm_config: VLMModelConfig,
...
#         )
#         module_name = f"base_model.model.model.layers.{transformer_layer.layer_id_}.{module_name}.{proj_name}"
...
#         # adapter configs
#         self.adapter_configs_: Dict[str, ] = {}
...
#         return VLMModel(model)

#     def init_adapter(
#         self, config: AdapterConfig, weight: Optional[Dict[str, torch.Tensor]] = None
#     ):
#         # Patch for MixLoRA
#         if isinstance(config, Mix) and config.act_fn_ is None:
...
#     ) -> Tuple[AdapterConfig, Dict[str, torch.Tensor]]:
